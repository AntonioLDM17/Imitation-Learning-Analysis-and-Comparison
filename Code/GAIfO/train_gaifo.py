...
def main():
    ...
    expert_s = torch.tensor(np.stack([t[0] for t in expert_transitions]), dtype=torch.float32)
    expert_s_next = torch.tensor(np.stack([t[1] for t in expert_transitions]), dtype=torch.float32)
    ...
    # Initialize the discriminator with flat_obs_dim
    discriminator = GAIfODiscriminator(flat_obs_dim, hidden_dim=64)
    ...
    for itr in range(num_iterations):
        ...
        rollout_s = torch.tensor(rollout_obs.reshape(-1, flat_obs_dim), dtype=torch.float32)
        rollout_s_next = torch.tensor(rollout_obs_next.reshape(-1, flat_obs_dim), dtype=torch.float32)
        ...
        for epoch in range(disc_epochs):
            ...
            disc_loss = loss_policy + loss_expert
